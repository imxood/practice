...
static void test_objects_init(void)
{
	...
	printk("Kernel objects initialized\n");
}
...
static void test_thread(int arg1, int arg2)
{
	...
	printk("Testing normal expiration of k_sleep()\n");
	...
	if (!sleep_time_valid(start_tick, end_tick, ONE_SECOND_ALIGNED)) {
		printk(" *** k_sleep() slept for %d ticks not %d.\n",
			 end_tick - start_tick, ONE_SECOND_ALIGNED);

		return;
	}

	printk("Testing: test thread sleep + helper thread wakeup test\n");
	...
	if (end_tick - start_tick > 1) {
		printk(" *** k_wakeup() took too long (%d ticks)\n",
			 end_tick - start_tick);
		return;
	}

	printk("Testing: test thread sleep + isr offload wakeup test\n");
	k_sem_give(&helper_thread_sem);   /* Activate helper thread */
	align_to_tick_boundary();
	...
	printk("Testing: test thread sleep + main wakeup test thread\n");
	...
	if (end_tick - start_tick > 1) {
		printk(" *** k_wakeup() took too long (%d ticks) at LAST\n",
			 end_tick - start_tick);
		return;
	}
	test_failure = false;
...
void test_sleep(void)
{
	...
	printk("Test thread started: id = %p\n", test_thread_id);
	...
	printk("Helper thread started: id = %p\n", helper_thread_id);
