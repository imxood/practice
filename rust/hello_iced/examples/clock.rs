    ...
    fn draw(&self, bounds: Rectangle, _cursor: Cursor) -> Vec<Geometry> {
        ...
        let clock = self.clock.draw(bounds.size(), |frame| {
            ...
            frame.with_save(|frame| {
                frame.rotate(hand_rotate(self.now.hour(), 12));
                frame.stroke(&short_hand, width_stroke);
            });

            frame.with_save(|frame| {
                frame.rotate(hand_rotate(self.now.minute(), 60));
                frame.stroke(&long_hand, width_stroke);
            });

            frame.with_save(|frame| {
                frame.rotate(hand_rotate(self.now.second(), 60));
                frame.stroke(&long_hand, width_stroke);
            });
        });
...
fn hand_rotate(n: u32, total: u32) -> f32 {
